import { createChart, updateChart } from "../Week_8/scatterplot.js";
...
function showData(data) {
  const columns = data.map(car => ({
    x: car.FuelConsumptionCity,
    y: car.FuelConsumptionComb
  }))
...
async function createNeuralNetwork(data) {
  ...
  nn.train({ epochs: 4 }, () => finishedTraining())
...
async function finishedTraining() {
  ...
  const testCar = { fuelConsumptionCity: 18 }
